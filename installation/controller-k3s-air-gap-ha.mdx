---
title: "Installing HA Netris Controller in Air-Gapped Environments"
sidebarTitle: "HA Air-Gapped Installation"
---
...
1. **Three Servers** in the same private network, each meeting the minimum [hardware requirements](/supported-switch-hardware) for K3s and Netris Controller.
...
Contact [Netris](https://www.netris.io/demo/) to acquire the air-gapped installation package, named **netris-controller-ha-v4.x.x.tar.gz**. This package contains everything you need for an HA deployment of Netris Controller on K3s, without internet connectivity.
...
<Tip>
  The North-South VIP is optional and can be configured at a later stage. The North-South VIP is typically used to provide access to the Netris API and for [ZTP functionality](/installation/ztp) for endpoints connected to the North-South fabric.
</Tip>
...
<Frame>
  <img src="/images/global-setting-local-repo.png" alt="global setting local repo" />
</Frame>

<Frame>
  <img src="/images/global-setting-local-repo-save.png" alt="global setting local repo save" />
</Frame>
...
<Frame>
  <img src="/images/one-liner-with-local-repo.png" alt="one liner with local repo" />
</Frame>
...
Contact [Netris](https://www.netris.io/demo/) to acquire the air-gapped upgrade package, named **netris-controller-ha-v4.x.x.tar.gz**. This package contains everything you need for an HA deployment of Netris Controller on K3s, without internet connectivity.
...
*If you're only upgrading the Local Netris Repository, you can skip this section and go directly to* [Section 3](/installation/controller-k3s-air-gap-ha#3-steps-to-upgrade-the-local-netris-repository)
...
4. **For each non-primary node**:
   ...
   1. **Cordon the node** to prevent new pods from being scheduled:
      ...
      ```
   2. **Drain the node** safely to relocate all running pods:

      ```bash
      kubectl drain <node-name> --ignore-daemonsets --delete-emptydir-data
      ```
   3. **Verify pods have been relocated**:

      ```bash
      kubectl get pods -A -o wide | grep <node-name>
      ```
   4. **Perform maintenance** on the node (updates, reboots, etc.)
   ...
   7. **Uncordon the node**:
      ...
      ```
   8. **Verify cluster health before proceeding to the next node**:
...
5. **For the node hosting the primary MariaDB**:
   ...
   1. **Double-check it's still hosting the primary** (as failover might have occurred):

      ```bash
      kubectl -nnetris-controller get maxscale netris-controller-ha-mariadb
      kubectl -nnetris-controller get pod <primary-pod-name> -o wide
      ```
   2. Follow the same cordon, drain, maintenance, and uncordon steps as above
