...
Infrastructure provisioning in Netris starts with Site resources. The Netris-Controller comes with the initial site `Default`. You can use it in your Terraform configuration files by getting its ID with the Terraform [Data source element](https://www.terraform.io/language/data-sources).
...
Or, you can create a new `Site` resource, [here](https://registry.terraform.io/providers/netrisai/netris/latest/docs/resources/site) is the detailed documentation with examples.
...
With the command above, we've defined 6 resources, 3 of the type of Allocation, 3 of the type of Subnet, each Subnet resource has a different purpose. For more details, get familiar with the IPAM [docs](/ipam).
...
<Note>
  For more information about all resources, how to create and manage them in Terraform, see the [provider's documentation](https://registry.terraform.io/providers/netrisai/netris/latest/docs).
</Note>
