...
<Frame>
  <img src="/images/EVPN-on-Host-high-level-diagram.svg" alt="EVPN on Host high level diagram" />
</Frame>
...
When a node with a Netris EVPN-on-Host agent installed is configured as a [Shared Endpoint](/server-cluster#shared-endpoints) in a given [Server Cluster](/server-cluster), Netris extends the relevant VXLAN segments into this host, and VMs or containers on these hosts can communicate with other VMs, containers, and Bare Metal nodes in the same VPC and V-Net.
...
<Frame>
  <img src="/images/EVPN-on-Host-packet-flow.svg" alt="EVPN on Host packet flow" />
</Frame>
...
<Frame>
  <img src="/images/EVPN-on-Host-Provisioning-V-net.svg" alt="EVPN on Host Provisioning V net" />
</Frame>
...
<Frame>
  <img src="/images/EVPN-on-Host-Management-V-net.svg" alt="EVPN on Host Management V net" />
</Frame>
...
- Include the switch ports (directly, using [labels](/labels), or through [Server Cluster](/server-cluster)) to which the EVPN-on-Host candidate servers are connected in the North-South fabric as **untagged**.
...
<Tip>
  Netris recommends using a [label](/labels) to dynamically include the correct switch ports in the Management [V-Net](/vnet).
</Tip>
...
<Frame>
  <img src="/images/EVPN-on-Host-install-agent-right-click.png" alt="EVPN on Host install agent right click" />
</Frame>
...
<Frame>
  <img src="/images/EVPN-on-Host-install-agent-oneliner.png" alt="EVPN on Host install agent oneliner" />
</Frame>
...
<Frame>
  <img src="/images/EVPN-on-Host-k8s01-eth9-underlay.png" alt="EVPN on Host k8s01 eth9 underlay" />
</Frame>

<br />

<Frame>
  <img src="/images/EVPN-on-Host-k8s01-eth10-underlay.png" alt="EVPN on Host k8s01 eth10 underlay" />
</Frame>
...
1. Add this node as a Shared Endpoint to that tenant's [Server Cluster](/server-cluster).

<Frame>
  <img src="/images/EVPN-on-Host-cluster-coke.png" alt="EVPN on Host cluster coke" />
</Frame>

<br />

2. Or add the appropriate switch ports to the V-Net using [label](/labels).

<Frame>
  <img src="/images/EVPN-on-Host-tenant-port-tagged.png" alt="EVPN on Host tenant port tagged" />
</Frame>
...
<Frame>
  <img src="/images/EVPN-on-Host-tenant-port-direct.png" alt="EVPN on Host tenant port direct" />
</Frame>
