---
title: "Netris Architecture"
---

This page describes the components of a Netris deployment and how they fit together. Before reading further, make sure you are comfortable with the four AI data center fabric roles (North-South, East-West, NVL72, OOB) introduced on the [Introduction](/introduction) page — every section below assumes that vocabulary.
...
- **NVIDIA BlueField DPUs** — managed by Netris as first-class network devices participating in the EVPN/VXLAN fabric for hard isolation (enforced on networking hardware) on the server itself. See [BlueField-3 DPUs](/bluefield-3-dpus).
- **NVIDIA UFM integration** — for automated InfiniBand partition (PKey) management on Quantum backend fabrics. See [UFM Integration](/netris-ufm-integration).
- **NVIDIA NMX integration** — for automated NVLink partition management on NVL72 rack-scale fabrics (GB200 NVL72 and GB300 NVL72). See [NMX Integration](/netris-nvlink-integration).
...
- **EVPN-on-Host** — extends the EVPN control plane into standard Linux hosts for VM and container tenant isolation. See [EVPN-on-Host](/evpn-on-host).
...
<Frame caption="Figure: High-level Netris architecture">
  <img src="/images/netris_controller_diagram.png" alt="High-level Netris architecture" />
</Frame>

- **Dedicated Controller Management Network (CMN).** The Netris Controller runs on a dedicated Controller Management Network (CMN). The CMN is a physically separate, non-Netris-managed network and is not one of the four AI data center fabric roles introduced on the [Introduction](/introduction) page (North-South, East-West, NVL72, OOB). Running the controller on the CMN prevents a circular dependency between the controller and the fabrics that the controller manages. See [Management Network Architecture](/netris-architecture#management-network-architecture) below for details.
...
You can learn more about SoftGate architecture and deployment scenarios in the [Netris SoftGate HS](/netris-softgate-hs) document.
...
<Frame caption="Figure: Hybrid OOB (hierarchical) management network architecture with separate CMN">
  <img src="/images/Hybrid-Management-Network-RA.svg" alt="Hybrid OOB (hierarchical) management network architecture with separate CMN" />
</Frame>
...
- **Audit Logging:** All requests processed by the Netris API are logged. Because the web GUI operates through the same API, UI activity is also captured in the audit logs. Logs can be retrieved programmatically through the controller API or inspected in the GUI. See [Graph Boards](/monitoring-observability/visibility) for details.
...
- Role-based access control ([Accounts](/accounts))
- Infrastructure configuration enforcement
- Audit logging ([Graph Boards](/monitoring-observability/visibility))
- Tenant isolation orchestration across Ethernet, InfiniBand, and NVLink fabrics
- Device management hardening via [Inventory Profiles](/inventory-profile)
...
With the architecture in mind, the natural next step is to see Netris in action. Open the [Try & Learn: GPU-as-a-Service network with NVIDIA Spectrum-X architecture](/try-learn/nvidia-spectrum-x-scenario) lab. The scenario runs in a Netris simulation environment — no real switches or GPU servers required — and walks through a complete deployment that exercises the four fabric roles, the vocabulary, and the components introduced above.

If you have already acquired Netris and are ready to deploy on real hardware, go to [Controller Installation](/installation/installation). If you want to confirm hardware support first, see the [Supported Platforms Matrix](/supported-platform-matrix) and [Supported Switch Hardware](/supported-switch-hardware).
